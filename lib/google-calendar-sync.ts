...
async function readGoogleEventsJson(
  response: Response
): Promise<GoogleEventsResponse | null> {
  try {
    return (
      await response.json()
...
function logGoogleEventsError(
  label: string,
  response: Response,
  result: GoogleEventsResponse | null
) {
  console.error(
    label,
    {
    ...
    for (
      ...
      if (
        ...
        if (
          ...
          logDatabaseError(
            "Google Calendar imported mapping rollback error:",
            rollbackDeleteError,
            {
              googleEventId,
              blockId,
        ...
        throw new Error(
          "No se ha podido completar la importación del evento de Google Calendar."
        );
      }
